...
testing_dir = os.path.join(os.getcwd(), 'chk-image/')
...
def prepare_data():
    ...
    for num, filename in enumerate(os.listdir(train_path)):
        if int(filename.split('_')[0])>=90:
            continue
        img = cv2.imread(train_path+filename)
        gray_img = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
        gray_img = cv2.equalizeHist(gray_img) # imp
...
def make_model():
    ...
    model.add(Conv2D(filters=96, input_shape=(img_w, img_h, img_channels), kernel_size=(11,11), strides=(4,4), padding='same'))
    model.add(Activation('relu'))
    # Max Pooling
    ...
    model.add(Dense(4096, input_shape=(img_w*img_h*img_channels,)))
    model.add(Activation('relu'))
...
def plot_graphs(history):
    # TASK: summarize history for accuracy
    ...
    # TASK: summarize history for loss
...
def make_pridiction(model):
    ...
    for filename in os.listdir(testing_dir):
        ...
        # TASK: show the inputs and predicted outputs
        try:
